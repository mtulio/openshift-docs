...
= Installing a cluster on user-provisioned infrastructure in AWS that uses local zones
...
//Do we need to remove the instructions to create separate worker nodes? It looks like the installation program might make the workers because we're creating a manifest for them.

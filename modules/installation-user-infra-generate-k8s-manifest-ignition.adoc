...
ifdef::aws,azure,ash,gcp,localzone[]
...
endif::aws,azure,ash,gcp,localzone[]
...
//The current AWS UPI method removes all of the `99_openshift-cluster-api_worker-machineset-* files. What is different about the local zones that means that we should retain them? Should we remove whatever the installation program creates and then specifically create this new file?
